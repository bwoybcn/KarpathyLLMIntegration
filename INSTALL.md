...
3. The vault template pre-registers these plugins in `community-plugins.json` so they'll be enabled once installed
4. **Check for plugin updates** — go to Settings > Community plugins > Check for updates. Dataview queries won't render if the plugin is outdated.
