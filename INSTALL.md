...
mkdir -p "$CLAUDE_HOME/skills/alphaxiv-paper-lookup"
...
cp source-files/skills/alphaxiv-paper-lookup/SKILL.md "$CLAUDE_HOME/skills/alphaxiv-paper-lookup/"
...
By default, knowledge base vaults are created at `D:\KnowledgeBases\`. To change this, set the `KB_ROOT` environment variable:

```bash
# In your shell profile (~/.bashrc, ~/.zshrc, etc.)
export KB_ROOT="$HOME/KnowledgeBases"
```

Or on Windows, set it via System Environment Variables to e.g. `C:\Users\you\KnowledgeBases`.

Alternatively, edit `~/.claude/scripts/kb_engine.py` directly and change the `DEFAULT_KB_ROOT` line.
...
3. Search and install **Terminal** — run Claude Code inside Obsidian
4. The vault template pre-registers these plugins in `community-plugins.json` so they'll be enabled once installed
5. **Check for plugin updates** — go to Settings > Community plugins > Check for updates. Dataview queries won't render if the plugin is outdated.
5. Optionally install these community plugins from Obsidian's settings:
